...
export default function SOSPage() {
  ...
  // Fetch user's active booking on mount
  useEffect(() => {
  ...
  const confirmTrigger = async () => {
    if (!activeBooking) { setError('No active booking. SOS requires an active booking.'); return; }
    setConfirming(false);
  ...
  if (triggered) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center space-y-4">
        ...
        <p className="text-on-surface-variant text-sm">Emergency services and admins have been notified. Stay safe.</p>
        <button onClick={() => router.back()} className="text-primary font-medium mt-4">← Go Back</button>
      </div>
    );
  }
      ...
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center space-y-4 px-4">
        ...
        <div className="flex gap-3 w-full max-w-xs">
          <button onClick={() => setConfirming(false)}
            className="flex-1 border border-outline-variant text-on-surface font-semibold py-3 rounded-xl bg-surface-container hover:bg-surface-container-high transition">Cancel</button>
          <button onClick={confirmTrigger}
            className="flex-1 bg-gradient-to-b from-red-500 to-red-700 text-white font-semibold py-3 rounded-xl shadow-[inset_0_1px_0_rgba(255,255,255,0.3),0_4px_0_#7f1d1d] hover:translate-y-[1px] active:translate-y-[3px] active:shadow-none transition-all">Confirm SOS</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] text-center space-y-8 px-4">
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        <h1 className="font-headline-md text-headline-md text-on-surface">Emergency Hub</h1>
      </div>
      ...
      {loadingBooking ? (
        ...
        <div className="glass-card rounded-xl p-4 text-amber-400 text-sm max-w-xs border border-amber-500/20">
          ⚠️ No active booking found. SOS requires an active booking (ACCEPTED, PICKING_UP, or IN_TRANSIT).
        </div>
      ) : (
        <>
          <div className="text-sm text-on-surface-variant">Active booking: #{activeBooking.id} ({activeBooking.status})</div>
          ...
          <div className="relative">
            ...
            <button
              ...
              onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') handlePressStart(); }}
              onKeyUp={(e) => { if (e.key === 'Enter' || e.key === ' ') handlePressEnd(); }}
