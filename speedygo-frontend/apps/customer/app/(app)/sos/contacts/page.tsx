...
export default function EmergencyContactsPage() {
  ...
  const save = async () => {
    ...
    const cleaned = contacts
      .map((c) => ({ name: c.name.trim(), phone: c.phone.trim(), email: c.email?.trim() || undefined }))
    ...
    <div className="space-y-5 pb-6 max-w-lg mx-auto animate-blur-fade-up">
      ...
      {contacts.length < 5 && (
        <button
          ...
          className="flex items-center gap-2 text-primary text-sm font-medium hover:text-tertiary transition-colors"
        >
