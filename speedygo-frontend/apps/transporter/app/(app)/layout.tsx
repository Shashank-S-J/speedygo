...
const NAV = [
  ...
  { href: '/history', label: 'History', icon: 'schedule' },
...
const SIDE_NAV = [
  ...
  { href: '/jobs', label: 'Job Board', icon: 'work' },
  { href: '/history', label: 'Trip History', icon: 'history' },
  { href: '/vehicles', label: 'My Vehicles', icon: 'local_shipping' },
  { href: '/earnings', label: 'Earnings', icon: 'account_balance_wallet' },
  { href: '/profile', label: 'Profile', icon: 'person' },
];
...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (!isAuthenticated || (user?.role && user.role !== 'TRANSPORTER')) {
      router.replace('/login');
    }
  }, [isAuthenticated, router, user?.role]);

  if (!isAuthenticated || (user?.role && user.role !== 'TRANSPORTER')) return (
    ...
    <div className="min-h-screen bg-background flex flex-col md:flex-row overflow-x-hidden">
      ...
      <nav className="hidden md:flex flex-col fixed left-0 top-0 h-full z-40 bg-slate-950/80 backdrop-blur-2xl w-64 border-r border-white/10 shadow-[20px_0_50px_rgba(30,27,75,0.4)]">
        <div className="p-6 flex items-center gap-4">
          <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-primary to-tertiary flex items-center justify-center shadow-[0_0_15px_rgba(76,215,246,0.3)]">
            <span className="material-symbols-outlined text-background font-bold" style={{ fontVariationSettings: "'FILL' 1" }}>local_shipping</span>
          </div>
          <div>
            <h1 className="text-xl font-bold text-white tracking-tight">SpeedyGo</h1>
            <p className="text-xs text-tertiary">Driver Portal</p>
          </div>
        </div>
      ...
      <header className="md:hidden sticky top-0 z-50 glass-panel border-b border-white/5 px-4 py-4 flex justify-between items-center">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href="/earnings" className="text-tertiary hover:text-white p-1.5 rounded-full hover:bg-white/5 transition-all">
